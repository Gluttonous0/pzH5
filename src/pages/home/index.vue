<template>
  ...
  <div class="header">
    ...
    <van-search v-model="searchText" placeholder="请输入搜索关键词" shape="round" width="calc(100vw - 128px)" />
  </div>
  <!-- 头部end -->
  ...
  <van-row justify="space-around">
    <van-col @click="handleShortCut(1, index)" span="11" v-for="(item, index) in indexData.nav2s" :key="item.id">
      <van-image :src="item.pic_image_url" />
    </van-col>
  </van-row>
  <!-- 快捷入口end -->
  ...
  <van-row
    class="yy-list"
    v-for="item in indexData.hospitals"
    justify="space-around"
    :key="item.id"
    @click="handleShortCut(2, item.id)"
  >
    <van-col span="6"> <van-image radius="5" width="100" height="90" :src="item.avatar_url" /></van-col>
    <van-col span="15" class="yy">
      <h3>{{ item.name }}</h3>
      <h5>{{ item.rank }} {{ item.label }}</h5>
      <p>{{ item.intro }}</p>
    </van-col>
  </van-row>
  <!-- 医院列表end -->
...
<script setup lang="ts">
  ...
  const indexData = reactive({
    hospitals: <H5Home.HospitalsParams[]>[],
    nav2s: <H5Home.NavsParams[]>[],
    navs: <H5Home.NavsParams[]>[],
    now: 0,
    slides: <H5Home.SlidesParams[]>[]
  })
  ...
  //快捷入口--------------
  const handleShortCut = (num: number, index: number) => {
    if (num === 1) {
